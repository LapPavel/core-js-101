...
function isCreditCardNumber(ccn) {
  const reverse = [...`${ccn}`].reverse().map((el) => +el);
  return reverse.reduce((acc, el, i) => {
    if ((i + 1) % 2 !== 0) return acc + el;
    if (el * 2 > 9) return acc + (+`${el * 2}`[0] + +`${el * 2}`[1]);
    return acc + el * 2;
  }, 0) % 10 === 0;
}
...
function isBracketsBalanced(str) {
  ...
  if (res.length % 2 === 0 && !res) return true;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i <= arr[0].length; i += 1) {
    const part = arr[0][i];
    if (!arr.every((el) => el[i] === part)) break;
    res += `${part}/`;
  }
  return res;
}
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < 3; i += 1) {
    if (
      [...new Set(position[i])].length === 1
      && !position[i].includes(undefined)
      && position[i].length > 2
    ) return position[i][0];
